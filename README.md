...
2. 호스트 환경에서 `ONNX` 또는 중간 표현으로 변환한다.
3. 작은 실제 입력 묶음으로 smoke 변환과 양자화 검증을 통과시킨다.
4. `RKNN` 산출물을 만든다.
5. `OrangePI RK3588`에 배치한다.
6. 실기기에서 단건 추론, 반복 추론, 카메라 입력, 지연 시간과 초당 처리 수를 확인한다.
7. 기준을 통과한 산출물만 canonical 경로로 승격한다.
...
- `runtime/`
  - `OrangePI RK3588` 실기기 실행, 카메라 입력, 성능 측정, 배포용 진입점
- `assets/`
...
- 기기에서 돌리는 코드, 카메라 입력, 실시간 루프, 성능 측정은 `runtime/`에 둔다.
